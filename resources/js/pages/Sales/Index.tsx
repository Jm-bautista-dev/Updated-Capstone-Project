...
export default function SalesIndex() {
  const { sales: paginatedSales = { data: [] }, filters = {}, stats = {}, branches = [], isAdmin } = usePage().props as unknown as SalesPageProps;
    ...
    <AppLayout breadcrumbs={[{ title: 'Sales & Orders', href: '/sales' }]}>
      ...
      <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-background font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 scroll-smooth">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
            <div className="bg-(--ops-surface-raised) border border-(--ops-border) rounded-[14px] p-4.5 relative overflow-hidden group shadow-sm flex flex-col justify-between min-h-25">
              ...
              <div>
                ...
                <p className="text-[8px] text-(--ops-text-faint) font-bold uppercase mt-1 tracking-widest">Awaiting preparation</p>
              </div>
            </div>
            ...
            <div className="bg-(--ops-surface-raised) border border-(--ops-border) rounded-[14px] p-4.5 relative overflow-hidden group shadow-sm flex flex-col justify-between min-h-25">
              ...
              <div>
                ...
                <p className="text-[8px] text-(--ops-text-faint) font-bold uppercase mt-1 tracking-widest">Actively being prepared</p>
              </div>
            </div>
            ...
            <div className="bg-(--ops-surface-raised) border border-(--ops-border) rounded-[14px] p-4.5 relative overflow-hidden group shadow-sm flex flex-col justify-between min-h-25">
              ...
              <div>
                ...
                <p className="text-[8px] text-(--ops-text-faint) font-bold uppercase mt-1 tracking-widest">Transactions processed today</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5 flex-1 min-w-0">
                ...
                {isAdmin && (
                  <Select value={branchFilter} onValueChange={handleBranchFilter}>
                    ...
                    <SelectContent className="bg-(--ops-surface-sunken) border-(--ops-border) rounded-xl text-foreground">
                ...
                <tbody className="divide-y divide-(--ops-border-subtle)">
                  {sales.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          <FiShoppingCart className="size-10 text-(--ops-text-faint) animate-bounce" />
                          <p className="text-base font-bold italic uppercase tracking-tighter text-(--ops-text-muted)">No orders found</p>
                          <p className="text-[10px] text-(--ops-text-faint) font-bold uppercase tracking-widest">No transaction records match the query</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    sales.map((sale) => (
                      <tr
                        key={sale.id}
                        className="hover:bg-(--ops-surface-sunken)/50 transition-colors duration-150 relative"
                      >
                        {/* Order Info */}
                        <td className={cn(
                          "px-5 transition-all",
                          ...
                          <div className="text-[10px] text-(--ops-text-muted) font-bold uppercase tracking-tight mt-1 flex items-center gap-1.5 flex-wrap">
                            ...
                            {sale.branch && (
                              <>
                                <span>•</span>
                                <span className="flex items-center gap-1"><FiMapPin className="size-3 text-(--ops-text-faint)" />{sale.branch.name}</span>
                              </>
                            )}
                          </div>
                        </td>
                        ...
                        {/* Type */}
                        <td className="px-5">
                          <Badge variant="outline" className="capitalize text-[8px] font-black tracking-widest rounded-lg bg-(--ops-surface-sunken) border-(--ops-border) text-(--ops-text-secondary)">
                            {sale.type}
                          </Badge>
                        </td>
                        ...
                        {/* Items */}
                        <td className="px-5 text-right font-mono font-bold text-xs text-(--ops-text-secondary)">
                        ...
                        <td className="px-5 text-right">
                          ...
                          <div className="text-[8px] uppercase font-black text-(--ops-text-faint) tracking-wider mt-0.5">{sale.payment_method}</div>
                        </td>
                          ...
                          <td className="px-5 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-52 bg-(--ops-surface-sunken) border-(--ops-border) rounded-xl p-1.5 shadow-2xl text-(--ops-text-secondary)">
                                  <DropdownMenuLabel className="text-[8px] font-black uppercase tracking-[0.2em] text-(--ops-text-muted) px-2.5 py-1.5 font-sans">Options</DropdownMenuLabel>
                                  <DropdownMenuItem
                                    className="rounded-[8px] py-1.5 px-2.5 text-xs font-bold gap-2 cursor-pointer hover:bg-(--ops-chip-active-bg) hover:text-foreground"
                                    onClick={() => {
                                  ...
                                  {sale.status !== 'cancelled' && (
                                    <>
                                      ...
                                      <DropdownMenuLabel className="text-[8px] font-black uppercase tracking-[0.2em] text-(--ops-text-muted) px-2.5 py-1.5">Correction Tools</DropdownMenuLabel>
                                      <DropdownMenuItem
                                        className="rounded-[8px] py-1.5 px-2.5 text-xs font-bold gap-2 cursor-pointer hover:bg-(--ops-hover) hover:text-rose-400 text-rose-500"
                                        onClick={() => {
                                          setSaleToVoid(sale);
          ...
          {/* PAGINATION */}
          {(paginatedSales.total || 0) > (paginatedSales.per_page || 0) && (
            <div className="p-5 bg-(--ops-surface-sunken)/30 border border-(--ops-border) rounded-[14px] flex flex-col md:flex-row justify-between items-center gap-4">
              <span className="text-[9px] font-black uppercase text-primary tracking-wider flex items-center gap-1.5">
                ...
                Showing {paginatedSales.from || 0}–{paginatedSales.to || 0} of {paginatedSales.total || 0} orders
              </span>
              ...
              <div className="flex items-center gap-1.5">
                <Button 
                  ...
                  onClick={() => paginatedSales.prev_page_url && router.get(paginatedSales.prev_page_url)} 
                ...
                <div className="flex items-center gap-1 mx-1.5">
                  {paginatedSales.links?.filter((l: PaginationLink) => !isNaN(Number(l.label))).map((link: PaginationLink) => (
                    <Button
                      key={link.label}
                ...
                <Button 
                  ...
                  onClick={() => paginatedSales.next_page_url && router.get(paginatedSales.next_page_url)} 
                  className="rounded-[8px] h-8 px-2 bg-(--ops-surface-sunken) border border-(--ops-border) text-(--ops-text-secondary) hover:text-foreground"
                >
      ...
      <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
        <DialogContent className="max-w-xl max-h-[95vh] overflow-y-auto p-0 border-none bg-transparent shadow-none">
          ...
          <div className="relative mx-auto w-100 print:w-full print-receipt-body">
            {/* Zigzag Top Edge */}
            <div className="h-4 w-full bg-(--ops-surface-sunken) overflow-hidden print:hidden" style={{ clipPath: 'polygon(0% 100%, 5% 0%, 10% 100%, 15% 0%, 20% 100%, 25% 0%, 30% 100%, 35% 0%, 40% 100%, 45% 0%, 50% 100%, 55% 0%, 60% 100%, 65% 0%, 70% 100%, 75% 0%, 80% 100%, 85% 0%, 90% 100%, 95% 0%, 100% 100%)' }} />

            <div className="bg-zinc-950 border border-(--ops-border) rounded-[14px] shadow-2xl p-0 relative overflow-hidden group text-foreground">
              {/* Brand Header */}
              <div className="bg-(--ops-surface-raised) p-8 border-b border-(--ops-border-subtle) border-dashed text-center space-y-4 relative overflow-hidden">
                <div className="absolute -top-4 -right-4 size-32 bg-primary/10 rounded-full blur-3xl opacity-50" />
                <div className="flex justify-center">
                  <div className="p-3 bg-(--ops-surface-sunken) rounded-3xl shadow-2xl ring-1 ring-zinc-800 flex items-center justify-center">
                ...
                <div className="flex flex-col items-center gap-2 pt-2">
                  <div className="flex items-center gap-2 px-3 py-1 bg-(--ops-chip-active-bg) rounded-full text-(--ops-text-secondary)">
                  ...
                  <p className="text-[10px] font-black text-(--ops-text-secondary) uppercase tracking-[0.2em]">{selectedSale?.branch?.name || 'Maki Desu Victoria'}</p>
                </div>
              </div>

              {/* Ticket Core Info */}
              <div className="p-6 space-y-6">
                <div className="flex justify-between items-start border-b border-(--ops-border-subtle) border-dashed pb-6">
                  <div className="space-y-1">
                    <p className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Receipt Number</p>
                  ...
                  <div className="text-right space-y-1">
                    <p className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Date & Time</p>
                    <p className="text-xs font-black text-foreground">
                      {selectedSale && format(new Date(selectedSale.created_at), 'MMM dd, yyyy')}
                    </p>
                    <p className="text-xs font-bold text-(--ops-text-muted)">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">
                      <FiHash className="size-2.5" /> Items
                    </div>
                    <div className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Value</div>
                  </div>
                  <div className="space-y-2.5">
                    {selectedSale?.items.map((item) => (
                      <div key={item.id} className="flex justify-between items-end p-2 rounded-xl hover:bg-(--ops-surface-sunken) transition-all">
                        <div className="space-y-1">
                          <p className="text-xs font-black text-foreground">{item.product.name}</p>
                          <p className="text-[10px] font-bold text-(--ops-text-muted)">{item.quantity} × {formatCurrency(item.unit_price)}</p>
                        </div>
                ...
                {/* Total breakdown */}
                <div className="pt-6 border-t-2 border-(--ops-border-subtle) border-dashed space-y-4">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-(--ops-text-secondary) px-2">
                    <span>Subtotal</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-(--ops-surface-sunken)/30 p-3.5 rounded-2xl border border-(--ops-border-subtle) space-y-1">
                    <p className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Paid via</p>
                    <p className="text-xs font-black uppercase tracking-tighter text-foreground">{selectedSale?.payment_method}</p>
                  </div>
                  <div className="bg-(--ops-surface-sunken)/30 p-3.5 rounded-2xl border border-(--ops-border-subtle) space-y-1">
                    <p className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Change Given</p>
                ...
                <div className="pt-6 border-t border-(--ops-border-subtle) border-dashed flex flex-col items-center gap-6 text-center">
                  <div className="flex items-center gap-4 bg-(--ops-surface-sunken)/30 border border-(--ops-border-subtle) p-3.5 rounded-2xl w-full">
                    <div className="size-10 rounded-xl bg-(--ops-surface-sunken) flex items-center justify-center shadow-sm">
                      <FiUserCheck className="size-5 text-primary" />
                    </div>
                    <div className="text-left">
                      <p className="text-[9px] font-black uppercase tracking-widest text-(--ops-text-muted)">Served By</p>
                  ...
                  <div className="space-y-3">
                    <div className="size-20 bg-(--ops-surface-sunken) p-2 rounded-xl border border-(--ops-border) mx-auto">
                      <div className="w-full h-full bg-zinc-950 rounded border border-dashed border-(--ops-border) flex items-center justify-center">
                        <div className="size-6 border-2 border-(--ops-border) rounded-sm" />
                      </div>
                    </div>
                    <div>
                      <p className="text-[8px] font-black uppercase tracking-widest italic text-(--ops-text-faint)">Secure Transaction Verified</p>
                      <p className="text-[7px] font-bold text-(--ops-text-faint) uppercase mt-0.5 font-mono">Order Index: {selectedSale?.id.toString().padStart(8, '0')}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Zigzag Bottom Edge */}
            <div className="h-4 w-full bg-(--ops-surface-sunken) overflow-hidden print:hidden" style={{ clipPath: 'polygon(0% 0%, 5% 100%, 10% 0%, 15% 100%, 20% 0%, 25% 100%, 30% 0%, 35% 100%, 40% 0%, 45% 100%, 50% 0%, 55% 100%, 60% 0%, 65% 100%, 70% 0%, 75% 100%, 80% 0%, 85% 100%, 90% 0%, 95% 100%, 100% 0%)' }} />

            {/* Print Quick Controls Floating */}
            <div className="absolute bottom-8 -right-25 group-hover:right-8 transition-all duration-500 print:hidden flex flex-col gap-2">
              ...
              <Button variant="outline" size="icon" className="size-10 rounded-2xl bg-(--ops-surface-sunken) border-(--ops-border) text-(--ops-text-secondary) hover:text-foreground" onClick={() => setIsDetailsModalOpen(false)}>
      ...
      <Dialog open={isVoidModalOpen} onOpenChange={setIsVoidModalOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none bg-transparent shadow-none">
          <div className="bg-zinc-950 border border-(--ops-border) rounded-[20px] overflow-hidden shadow-2xl text-foreground">
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <DialogDescription className="text-xs font-semibold text-(--ops-text-secondary) leading-normal">
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                <Button
                  variant="ghost"
                  className="h-10 rounded-[10px] font-black uppercase tracking-wider text-[10px] text-(--ops-text-secondary) hover:bg-(--ops-surface-sunken)"
