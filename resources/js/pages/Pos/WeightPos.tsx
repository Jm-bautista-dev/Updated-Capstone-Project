...
interface WeightPosPageProps {
  inventory?: WeightInventoryItem[];
  [key: string]: unknown;
}
...
export default function WeightPos() {
  ...
  const filteredInventory = useMemo(() => {
    return inventory.filter((item: WeightInventoryItem) => 
  ...
  const handleSelectItem = (item: WeightInventoryItem) => {
    setSelectedItem(item);
    setData('item_id', String(item.id));
  };
      ...
      <div className="flex h-screen overflow-hidden bg-muted/20">
        ...
        <div className="flex-1 flex flex-col p-6 space-y-6 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pr-2">
            <AnimatePresence>
              {filteredInventory.map((item: WeightInventoryItem) => (
                <motion.button
                  key={item.id}
                  layout
        ...
        <div className="w-96 bg-background border-l p-8 flex flex-col space-y-8">
          ...
          {selectedItem ? (
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col space-y-8">
              ...
              <div className="bg-primary/5 rounded-3xl p-6 border border-primary/10 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Badge className="bg-[#e75480]/10 text-primary border-none text-[9px] font-black uppercase">{selectedItem.unit} Unit</Badge>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 opacity-40 italic">
              ...
              <p className="text-sm font-bold max-w-48">Select an item from the left to begin the weight-based order process.</p>
            </div>
          )}
        </div>
      </div>
