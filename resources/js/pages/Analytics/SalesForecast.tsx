...
const ChartTooltip = ({ active, payload }: any) => {
 ...
 return (
 <div className="bg-background p-4 shadow-2xl rounded-2xl border ring-1 ring-black/5 min-w-[200px]">
 <p className="text-[10px] font-black uppercase text-muted-foreground mb-3 border-b pb-2">{d?.date}</p>
 <div className="space-y-2">
 {d?.actual != null && (
 <div className="flex justify-between gap-4">
 <span className="text-[11px] font-bold text-muted-foreground">Actual</span>
 <span className="text-[11px] font-black text-foreground">{formatCurrency(d.actual)}</span>
 </div>
 )}
 {d?.moving_avg != null && (
 <div className="flex justify-between gap-4">
 <span className="text-[11px] font-bold text-amber-500">Moving Avg</span>
 <span className="text-[11px] font-black text-amber-600">{formatCurrency(d.moving_avg)}</span>
 </div>
 )}
 {d?.predicted != null && (
 <>
...
function ConfidenceBadge({ score }: { score: number }) {
 ...
 const config = {
 high: { label: 'High Confidence', cls: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20', icon: FiShield },
 medium: { label: 'Medium Confidence', cls: 'bg-amber-500/10 text-amber-600 border-amber-500/20', icon: FiMinus },
 low: { label: 'Low Confidence', cls: 'bg-rose-500/10 text-rose-600 border-rose-500/20', icon: FiAlertTriangle },
 }[level];
...
export default function SalesForecast() {
 ...
 <SelectTrigger className="w-32 bg-transparent border-none shadow-none focus:ring-0 text-xs font-bold uppercase text-foreground">
 <SelectValue />
 </SelectTrigger>
 <SelectContent>
 ...
 <SelectTrigger className="w-44 bg-transparent border-none shadow-none focus:ring-0 text-xs font-bold uppercase text-foreground">
 ...
 <h3 className={cn('text-2xl font-black', trend?.slope >= 0 ? 'text-emerald-600 ' : 'text-destructive ')}>
 ...
 <div className="mt-4">
 <Badge variant="outline" className={cn('border-none font-black text-[10px] uppercase', trend?.percentage >= 0 ? 'bg-emerald-500/10 text-emerald-700 ' : 'bg-rose-500/10 text-rose-700 ')}>
 ...
 <p className="text-2xl font-black text-foreground tabular-nums">{(confidence ?? 0).toFixed(1)}%</p>
 ...
 {/* Main Chart */}
 <Card className="lg:col-span-2 border-none shadow-sm ring-1 ring-border bg-card">
 ...
 <XAxis dataKey="date" stroke="currentColor" className="text-muted-foreground" fontSize={10} fontWeight="bold" axisLine={false} tickLine={false} minTickGap={30} />
 <YAxis stroke="currentColor" className="text-muted-foreground" fontSize={10} fontWeight="bold" axisLine={false} tickLine={false} tickFormatter={v => `₱${v >= 1000 ? (v / 1000).toFixed(1) + 'k' : v}`} />
 ...
 {/* 7-Day Table */}
 <Card className="border-none shadow-sm ring-1 ring-border bg-card overflow-hidden">
 ...
 <tr>
 <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-muted-foreground">Date</th>
 <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-muted-foreground text-right">Forecast</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border">
 {forecast.map((f, i) => (
 <tr key={i} className="hover:bg-muted/30 :bg-muted/30 transition-colors group">
 <td className="px-5 py-3">
 <div className="flex items-center gap-2">
 <div className="size-1.5 rounded-full bg-primary group-hover:scale-125 transition-transform" />
 <div>
 <p className="font-bold text-foreground text-xs">{format(parseISO(f.date), 'EEE, MMM d')}</p>
 ...
 >
 <p className="text-[11px] font-semibold text-foreground leading-relaxed">{insight}</p>
 </motion.div>
 ))}
