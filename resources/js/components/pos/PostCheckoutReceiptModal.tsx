...
export const PostCheckoutReceiptModal: React.FC<PostCheckoutReceiptModalProps> = ({
    ...
    useEffect(() => {
        ...
        if (initialStatus === 'failed') {
            setStatusMessage('Receipt printing failed. Please check the USB printer connection.');
        } else if (initialStatus === 'bridge_offline') {
            setStatusMessage('Thermal print bridge is offline. You can print via the browser.');
    ...
    const handleRetrySilentPrint = async () => {
        ...
        setIsRetrying(true);
        setStatus('printing');
        setStatusMessage('Spooling receipt to USB printer...');

        try {
            const res = await sendToLocalPrintBridge(printJob);
            if (res.success) {
                setStatus('success');
                setStatusMessage('Receipt printed successfully to USB thermal printer.');
                toast.success(`✓ Thermal receipt sent to printer for #${orderNum}`);
            } else {
                setStatus('failed');
                setStatusMessage(res.message || 'Receipt printing failed. Please check the USB printer connection.');
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-xl max-h-[92vh] flex flex-col p-0 overflow-hidden bg-white dark:bg-[#151518] border-[#F8C8DC]/60 dark:border-white/10 text-[#3D2C2E] dark:text-zinc-100 font-['Outfit'] shadow-2xl">
                    ...
                    <div className="mt-4">
                        {status === 'success' && (
                            <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/40 text-emerald-800 dark:text-emerald-300 text-xs font-semibold">
                                ...
                                <span>{statusMessage || 'Receipt printed successfully to USB printer.'}</span>
                            </div>
                        )}

                        {status === 'failed' && (
                            <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/40 text-amber-900 dark:text-amber-200 text-xs space-y-1">
                                <div className="flex items-center gap-2 font-bold text-amber-800 dark:text-amber-300">
                                    <FiAlertTriangle className="size-4 shrink-0 text-amber-600 dark:text-amber-400" />
                                    <span>Receipt printing failed</span>
                                    <span className="text-[10px] font-normal text-amber-700 dark:text-amber-400">
                                        ({isBridgeConnected ? 'Bridge Spooler Online' : 'Bridge Spooler Offline'})
                                    </span>
                                </div>
                                <p className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300/90 pl-6">
                                    Please check the USB printer connection. Make sure the printer is turned on, has 58mm paper loaded, and is recognized by the operating system.
                                </p>
                            </div>
                        )}
                        ...
                        {status === 'bridge_offline' && (
                            <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/40 text-amber-900 dark:text-amber-200 text-xs space-y-1">
                                <div className="flex items-center gap-2 font-bold text-amber-800 dark:text-amber-300">
                                    <FiAlertTriangle className="size-4 shrink-0 text-amber-600 dark:text-amber-400" />
                                    <span>Thermal Print Bridge Offline</span>
                                </div>
                                <p className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300/90 pl-6">
                                    The local print bridge is not currently running. You can click <strong>Print via Browser</strong> to print using your Windows-installed USB printer, or start the bridge service.
                                </p>
                            </div>
                        )}
                        ...
                        {status === 'printing' && (
                            <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200/80 dark:border-blue-800/40 text-blue-800 dark:text-blue-300 text-xs font-semibold">
                                ...
                                <span>{statusMessage || 'Spooling receipt to USB printer...'}</span>
                            </div>
                        )}
                    </div>
                </DialogHeader>
                ...
                <DialogFooter className="p-4 sm:p-5 bg-white dark:bg-[#171719] border-t border-[#F8C8DC]/40 dark:border-white/10 shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
                    <div className="flex items-center gap-2 flex-1">
                        ...
                        <Button
                            ...
                            title="Retry sending receipt silently through the local USB thermal print bridge"
                        >
                            <FiRotateCw className={`size-3.5 ${isRetrying ? 'animate-spin text-[#E75480]' : ''}`} />
                        ...
                        <Button
                            ...
                            title="Print using the browser's native print dialog to your installed USB thermal printer"
                        >
