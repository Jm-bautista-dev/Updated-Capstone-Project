...
const createBranchDivIcon = (branchName: string) => {
    const html = `
        <div class="relative flex items-center justify-center size-8 rounded-xl bg-gradient-to-tr from-[#E75480] to-[#FF4F81] text-white border-2 border-white shadow-md">
        ...
        if (branchCoords && branchCoords[0] && branchCoords[1]) {
            if (branchMarkerRef.current) {
                ...
                branchMarkerRef.current = L.marker(branchCoords, {
                    icon: createBranchDivIcon(branchName),
                })
